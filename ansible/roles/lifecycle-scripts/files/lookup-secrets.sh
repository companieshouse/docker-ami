...
function processPropertyFile() {
  ...
  KEYS_AND_PATHS_TO_LOOKUP=$( grep "@/" ${PROPERTIES_FILE} )

  for KEY_AND_PATH_TO_LOOKUP in ${KEYS_AND_PATHS_TO_LOOKUP}
  do
    PATH_TO_LOOKUP=$(echo ${KEY_AND_PATH_TO_LOOKUP} |  sed 's/.*["'\'']@\(\/.*\)["'\'']$/\1/g')
    KEY=$( echo ${KEY_AND_PATH_TO_LOOKUP} | sed 's/=.*//') 
